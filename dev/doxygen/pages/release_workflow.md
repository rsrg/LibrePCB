...
For every officially released version we create a Git tag on the corresponding
release branch. The tag name contains all three numbers of the application's
version number. Examples: "0.1.0-rc1", "0.1.0", "1.2.0", "1.2.1"
...
On the `master` branch, no translation files are checked into our source
repository because they change very often (avoid commit spam). But on release
branches, the translations are checked into the repository so they are always
available when building official releases (important for packagers). Of course
these translations are based on the Transifex resource for the corresponding
major version, *not* on the latest `librepcb.ts` resource.
...
published at https://download.librepcb.org. The website http://librepcb.org
contains direct download links to the corresponding files.
...
6. On the release branch, add in the translation files and commit.
...
9. If the artifacts are working properly, add and push a new tag with the name
   ...
   tag must always have three numbers (i.e. "0.1.0" instead of "0.1")!
